...
export function compareVariantOutputs(
  ...
  const sharedFiles = firstOutput
    ? firstOutput.fileList
        .filter((file) => fileSetUniverse.every((set) => set.has(file)))
        .sort((a, b) => a.localeCompare(b))
    : [];

  const keyedUniqueFiles = outputs.map((output): [string, string[]] => {
  ...
  const uniqueFilesByVariant: Record<string, string[]> = Object.fromEntries(
    keyedUniqueFiles.sort(([a], [b]) => a.localeCompare(b))
  );
  ...
  for (let i = 0; i < outputs.length; i += 1) {
    ...
    for (let j = i + 1; j < outputs.length; j += 1) {
      const b = outputs[j];
      if (!b || a.directoryHash === b.directoryHash) continue;
